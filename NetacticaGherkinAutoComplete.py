...
def plugin_loaded():
    global settings
    settings = sublime.load_settings('NetacticaGherkinAutoComplete.sublime-settings')


class NetacticaGherkinAutoComplete(sublime_plugin.EventListener):

    def on_query_completions(self, view, prefix, locations):

        nsGherkinType = view.substr(view.line(view.sel()[0]))
        if "Given " in nsGherkinType:
            return settings.get("given", [])
